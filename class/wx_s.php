...
class wx_s
{
  ...
  private $shopid;
	 ...
	 //  失败     {"errcode":40018,"errmsg":"invalid button name size"}
	  function __construct($wxtoken = '', $wxappid = '', $wxsecret = '', $shopid = 0){
	  	$this->wxtoken =  $wxtoken ? $wxtoken : Mysite::$app->config['wxtoken'];
	  	$this->wxappid =  $wxappid ? $wxappid : Mysite::$app->config['wxappid'];
	  	$this->wxsecret = $wxsecret ? $wxsecret : Mysite::$app->config['wxsecret'];
      $this->shopid = $shopid;
    }
    //获取token
   function checktoken(){
