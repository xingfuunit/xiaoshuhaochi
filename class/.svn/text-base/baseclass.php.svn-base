  	   ...
  	   switch($siteps['pstype']){
  	     ...
  	     case '4'://地图
  	       if($locationtype == 1){
  	       	  ...
  	       	  if(empty($shopinfo['pradiusvalue'])){
  	       	      $backdata['pscost'] = 0;
  	       	      $backdata['canps'] = 0;
  	       	  }
  		...
  		switch($shopps['pstype']){
  	     ...
  	     break;
             case '6':
                 $full_pay =  $shopps['psvalue1'];
                 $need_pay = $shopps['psvalue2']; 
               $backdata['pscost'] =  empty($shopps['psvalue2'])?0:$shopps['psvalue2'];
               $backdata['full_pay'] = empty($shopps['psvalue1'])?0:$shopps['psvalue1'];
               $backdata['canps'] = 1;
               break;
  	     default://不在 所列举 范围内则为0；
  	        $backdata['pscost'] =  0;
