...
class wx_b_shop{
	...
	private $tablepre;
	private $result;
	function __construct($wxobj, $shopid){
	  global $Mconfig;
	...
	function text(){
		...
		if(count($tempinfo) == 2){
			//修改密码
			 $wxuser = $this->mysql->select_one("select * from ".$this->tablepre."shop_wxuser where openid='".$this->obj->FromUserName."'");
			 ...
			 $data['uid'] = $info['uid'];
			 $data['is_bang'] = 1;
			 $this->mysql->update($this->tablepre.'shop_wxuser',$data,"openid='".$this->obj->FromUserName."'");
			 //删除默认绑定帐号
		   ...
		   $this->showorder();
			...
			if(!empty($controller) && strlen($controller) <  10){
			     $backinfo= $this->mysql->select_one("select * from ".$this->tablepre."shop_wxback where code = '".$controller."' and shopid=$this->shopid");
			     if(!empty($backinfo)){
  ...
  function GetDistance($lat1, $lng1, $lat2, $lng2, $len_type = 1, $decimal = 2)
  {
	...
	function event(){
       ...
       $arr['loginip'] ='';
       $arr['group'] = 10;
       $ehckinfo = $this->mysql->select_one("select * from ".$this->tablepre."shop_wxuser where openid='".$this->obj->FromUserName."'");
       if(empty($ehckinfo)){
          ...
          $data['shopid'] = $this->shopid;
		      $this->mysql->insert($this->tablepre.'shop_wxuser',$data);
		      //logwrite('写用户信息');
		   ...
		   if(empty($code)){
		     ...
		     $this->showorder();
		   	  ...
		   	  $backinfo= $this->mysql->select_one("select * from ".$this->tablepre."shop_wxmenu where code = '".$code."' ");
			     if(!empty($backinfo)){
			     	 //logwrite('调用自动回复');
        ...
        if(!empty($userinfo)){
        	  ...
        	  $this->mysql->update($this->tablepre.'shop_wxuser',$memberdata,"uid='".$userinfo['uid']."'");
             $shopinfo= $this->mysql->select_one("select * from ".$this->tablepre."shop where uid = '".$userinfo['uid']."' ");
             /*
             if(!empty($shopinfo)){
             	  $this->Rtext('定位');
	...
	function userinfo(){
	   $info = $this->mysql->select_one("select * from ".$this->tablepre."shop_wxuser as a left join ".$this->tablepre."member as b on a.uid = b.uid   where a.openid='".$this->obj->FromUserName."' ");
	    return $info;
	}
	//回复信息函数
