	  ...
	  if(datas.flag == false){    
	     if($('#addShopping').html() != undefined){
    	    ...
    	    $.each(datas.content.gzdata, function(i,val){    
    	    	  $('.bottomprise').append('<p  class="colgreen_'+i+' ">'+val+' </p>');
    	    });
...
function  orderSubmit(){ 
   ...
   $.ajax({         //script定义
            ...
            dataType: "json",
            async:true,
            data:{shopid:shopid,'remark':$('#remark').val(),'username':$('#username').val(),'mobile':$('#mobile').val(),'addressdet':$('#addressdet').val(),'minit':$("#DeliveryTime").find("option:selected").val(),'dikou':$('#jifen').val(),'juanid':$('#juanid').val(),'paytype':$("input[name='paytype']:checked").val()},
 ...
 function save_gust_address () {
	...
	if($('#addressdet').val()  == $('#addressdet').attr('data')){
	  ...
	  return false;
   	 ...
   	 url = siteurl+'/index.php?controller=html5&action=saveGustAddress';
        var submitData={
