$(function(){ 
 	   if(is_open == true){
 	     	$.getScript(siteurl+'/index.php?controller=html5&action=getshop&shopid='+shopid);   
 	   }
 	   $('#loading').hide(); 
 }); 
...
$('#favor').live("click", function() {  
	if($(this).hasClass('scno') == true){
		 $.getScript(siteurl+'/index.php?controller=html5&action=addcollect&collectid='+shopid);
	}else{
		 
		 $.getScript(siteurl+'/index.php?controller=html5&action=delcollect&collectid='+shopid);
	}
...
function gooodsdata(){
	$.getScript(siteurl+'/index.php?controller=html5&action=getgoods&shopid='+shopid);   
}
...
$('.clkitm').bind("click", function() {   
	$('#loading').show();
         ...
         $('.sortbox').find('li').eq(0).trigger("click");
          $.getScript(siteurl+'/index.php?controller=html5&action=cart&shopid='+shopid);
   }else{ 
   	   //is_open = false;
       // error($('.wm-container'),datas);
   }   
