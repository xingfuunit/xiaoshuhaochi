	...
	$.ajax({
     ...
     dataType: 'json',success: function(content) {   
      ...
      // 
       ...
       if(content.msg.length < 1){
       	var linkurl = siteurl+'/index.php?controller=site&action=setlocationlink&areaid='+areaid; 
...
function dosearchmap(pagenum){
	if($('#run-searchAddress').val() == $('#run-searchAddress').attr('tip') || $('#run-searchAddress').val() == ''){
	  alert('请录入查询条件');
	   ...
	   map.clearOverlays();
	   $('.search_result').show();
	   var  tempurlss = siteurl+'/index.php?controller=site&action=ajaxbaidu&random=@random@';  //  searchvalue  cityname
