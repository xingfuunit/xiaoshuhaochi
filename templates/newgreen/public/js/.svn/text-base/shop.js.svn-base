...
{
	var url = siteurl+'/index.php?controller=site&action=addcollect&collectid='+$(obj).attr('data')+'&type=1'+'&@random@'; 
	$.ajax({
     type: 'post',
     async:false, 
...
function myFavorite2(obj)
{
	...
	var url = siteurl+'/index.php?controller=site&action=addcollect&collectid='+$(obj).attr('data')+'&type=0'+'&@random@'; 
	$.ajax({
     type: 'post',
     async:false, 
     url: url.replace('@random@', 1+Math.round(Math.random()*1000)), 
...
$(".hcl_collection_btn_div").live("click", function() {   
	 ...
	 var obj = $(this);
		var url = siteurl+'/index.php?controller=site&action=delcollect&collectid='+$(this).attr('food_id')+'&type=1'+'&@random@'; 
	$.ajax({
     type: 'post',
     async:true, 
     url: url.replace('@random@', 1+Math.round(Math.random()*1000)), 
...
function getLiuyan(page)
{
	...
	var url = siteurl+"/index.php?controller=ask&action=shopask";
...
function getPingjia(page)
{
	...
	var url = siteurl+"/index.php?controller=shop&action=showping";
	$.post(url, {'shopid':$('#nowshopid').val(),'type':'shop','page':page},function (data, textStatus){
