...
function addonedish(gid,tshopid,num,obj){
    var url= siteurl+'/index.php?controller=site&action=addcart&goods_id='+gid+'&shopid='+tshopid+'&num=1&plate=1&datatype=json&random=@random@';
...
function removeonedish(gid,tshopid,num){
     $('#loading').show();
     url = siteurl+'/index.php?controller=site&action=downcart&goods_id='+gid+'&shopid='+tshopid+'&num=1&plate=1&datatype=json&random=@random@';
         url = url.replace('@random@', 1+Math.round(Math.random()*1000));
    var bk = ajaxback(url,'');
        if(bk.flag == false){
           freshcart();
...
//刷新购物车
function freshcart(){
        url = siteurl+'/index.php?controller=site&action=smallcatding&shopid='+shopid+'&plate=1&random=@random@';
        url = url.replace('@random@', 1+Math.round(Math.random()*1000));
